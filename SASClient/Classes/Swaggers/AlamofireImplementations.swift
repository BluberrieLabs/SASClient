        ...
        default:
            validatedRequest.responseJSON(options: .allowFragments) { response in
                ...
                if response.result.isFailure {
                    NSLog("Called defaultesponse")
                    
                    var bErr: String?
                    
                    NSLog("Got response \(response.response?.statusCode)")
                    
                    if let data = response.data {
                        do {
                            let berr = try JSONSerialization.jsonObject(with: data) as? [String: String]
                            bErr = (berr?["error"]!)!
                            print("Failure Response: "+(berr?["error"]!)!)
                        } catch {
                            //too bad
                        }
                    }
                    if (bErr != nil) {
                        NSLog("Called custom completion")
                        
                        let fooErr = NSError(domain: "localhost", code: response.response?.statusCode ?? 500, userInfo: [NSLocalizedDescriptionKey: bErr!])
                        
                        NSLog(fooErr.localizedDescription)
                        
                        completion(nil, ErrorResponse.Error(response.response?.statusCode ?? 500, nil, fooErr))
